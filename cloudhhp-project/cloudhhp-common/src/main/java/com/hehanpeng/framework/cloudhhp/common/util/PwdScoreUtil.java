...
/**
    ...
    分数不需要达到最低字元即可计算.
 */
public class PwdScoreUtil {
	
	// 不合格
	public static final int SCORE_LEVEL_UNQUALIFIED = 0;
	
	// 低级
	public static final int SCORE_LEVEL_LOW = 1;
	
	// 中级
	public static final int SCORE_LEVEL_INTERMEDIATE = 2;
	
	// 高级
	public static final int SCORE_LEVEL_SENIOR = 3;
    ...
    public PwdScoreUtil(String psw){
    ...
    public int seriesNum2Three(){
        ...
        for (int i = 0; i < c.length-2; i++) {
            if (Pattern.compile("[0-9]").matcher(c[i]+"").find()) {
                if (((c[i+1]==c[i]+1) && (c[i+2]==c[i]+2))||((c[i+1]==c[i]-1) && (c[i+2]==c[i]-2))) {
                    j++;
                }
            }
        }
    ...
    public int calScoreResult() {
    	int score = calScore();
    	if(score <= 10) {
    		return SCORE_LEVEL_UNQUALIFIED;
    	}else if(score > 10 && score <= 50) {
    		return SCORE_LEVEL_LOW;
    	}else if(score > 50 && score <= 80) {
    		return SCORE_LEVEL_INTERMEDIATE;
    	}else {
    		return SCORE_LEVEL_SENIOR;
    	}
    }
    public static void main(String[] args) {
		PwdScoreUtil t=new PwdScoreUtil("123456");
		System.out.println("最终得分"+t.calScore());
	}
}